const en = {
  ...
  narrative_generator: {
    ...
    experience: {
      rookie: ['A fresh graduate from the academy.', 'First time flying this route.', 'Still learning the ropes.'],
      amateur: ['A few hundred hours logged.', 'Comfortable with the basics.', 'Looking to build hours.'],
      intermediate: ['Seasoned first officer.', 'Thousands of hours in the sky.', 'Seen a fair share of weather.'],
      advanced: ['Veteran captain.', 'Decades of experience.', 'Nothing surprises you anymore.'],
      pro: ['Legendary aviator.', 'Written textbooks on flight dynamics.', 'The best of the best.'],
      devil: ['Test pilot for experimental craft.', 'You laugh at danger.', 'Flying is your second nature.']
    },
    flight_plan: [
      'Transport ${pax} souls from ${departure} to ${arrival}. Smooth skies expected.',
      'Scheduled service from ${departure} to ${arrival}. Heavy traffic reported en route.',
      'Red-eye flight from ${departure} to ${arrival}. Passengers are sleeping.',
      'Charter flight from ${departure} to ${arrival}. VIPs on board.'
    ],
    difficulty: {
      rookie: ['Clear skies, light winds.', 'Standard instrument departure.', 'No significant hazards.'],
      amateur: ['Moderate turbulence expected.', 'Busy airspace.', 'Crosswind landing likely.'],
      intermediate: ['Storm front approaching.', 'Systems showing minor fluctuations.', 'Low visibility at destination.'],
      advanced: ['Severe icing conditions.', 'Multiple system failures possible.', 'Engine performance degraded.'],
      pro: ['Catastrophic weather.', 'Critical system failures imminent.', 'Zero visibility.'],
      devil: ['The laws of physics are testing you.', 'Everything that can go wrong, will go wrong.', 'Good luck.']
    }
  },
  narrative: {
  ...
  ui: {
    ...
    flight_computer: {
        ...
        radio: {
            ...
            template: {
                req_clearance: { label: 'Request IFR Clearance', text: '${station}, ${callsign} requesting IFR clearance to ${destination} via ${sid}, ${altitude}, ${route}.' },
                req_startup: { label: 'Request Startup', text: '${station}, ${callsign} requesting startup and pushback.' },
                req_pdc: { label: 'Request PDC', text: '${station}, ${callsign} requesting Pre-Departure Clearance.' },
                req_alt_brief: { label: 'Alternate Airport Brief', text: '${station}, ${callsign} confirming alternate airport ${alternate}.' },
                req_runway_confirm: { label: 'Confirm Departure Runway', text: '${station}, ${callsign} requesting confirmation of departure runway.' },
                ack_clearance: { label: 'Acknowledge Clearance', text: 'Cleared to ${destination} via ${sid}, climb ${altitude}, squawk ${squawk}, ${callsign}.' },
                ack_squawk: { label: 'Readback Squawk', text: 'Squawk ${code}, ${callsign}.' },
                report_unable: { label: 'Report Unable', text: 'Unable ${instruction}, request alternative, ${callsign}.' },
                req_taxi: { label: 'Request Taxi', text: '${station}, ${callsign} ready for taxi to runway ${runway}.' },
                req_pushback: { label: 'Request Pushback', text: '${station}, ${callsign} requesting pushback facing ${direction}.' },
                req_deice: { label: 'Request De-icing', text: '${station}, ${callsign} requesting de-icing at current position.' },
                req_hold_short: { label: 'Hold Short Instructions', text: '${station}, ${callsign} requesting hold short instructions.' },
                req_cross_runway: { label: 'Request Runway Crossing', text: '${station}, ${callsign} requesting to cross runway ${runway}.' },
                req_return_gate: { label: 'Return to Gate', text: '${station}, ${callsign} requesting return to gate due to ${reason}.' },
                report_ground_issue: { label: 'Report Ground Issue', text: '${station}, ${callsign} reporting ${issue}.' },
                req_takeoff: { label: 'Ready for Departure', text: '${station}, ${callsign} ready for departure runway ${runway}.' },
                req_lineup: { label: 'Line Up and Wait', text: '${station}, ${callsign} ready to line up and wait runway ${runway}.' },
                req_land: { label: 'Request Landing', text: '${station}, ${callsign} inbound for landing runway ${runway}.' },
                req_go_around: { label: 'Going Around', text: 'Going around, ${callsign}.' },
                req_missed_approach: { label: 'Missed Approach', text: 'Missed approach, ${callsign}.' },
                req_short_approach: { label: 'Request Short Approach', text: '${station}, ${callsign} requesting short approach.' },
                abort_takeoff: { label: 'Abort Takeoff', text: 'Stopping, ${callsign}. Rejecting takeoff due to ${reason}.' },
                report_windshear: { label: 'Report Wind Shear', text: 'Wind shear reported on ${location}, ${callsign}.' },
                inf_checkin: { label: 'Check In', text: '${station}, ${callsign} passing ${altitude} for ${target_alt}.' },
                req_alt_change: { label: 'Request Altitude Change', text: '${station}, ${callsign} requesting ${altitude}.' },
                req_vectors_ils: { label: 'Request Vectors ILS', text: '${station}, ${callsign} requesting vectors for ILS approach runway ${runway}.' },
                req_visual: { label: 'Request Visual Approach', text: '${station}, ${callsign} requesting visual approach runway ${runway}.' },
                req_holding: { label: 'Request Holding', text: '${station}, ${callsign} requesting holding at ${waypoint}.' },
                req_weather_dev: { label: 'Weather Deviation', text: '${station}, ${callsign} requesting deviation ${direction} due to weather.' },
                req_step_climb: { label: 'Request Step Climb', text: '${station}, ${callsign} requesting step climb to ${altitude}.' },
                req_direct: { label: 'Request Direct', text: '${station}, ${callsign} requesting direct ${waypoint}.' },
                req_reroute: { label: 'Request Reroute', text: '${station}, ${callsign} requesting reroute due to ${reason}.' },
                req_turbulence: { label: 'Report Turbulence', text: '${station}, ${callsign} reporting ${severity} turbulence.' },
                inf_mayday: { label: 'MAYDAY', text: 'MAYDAY MAYDAY MAYDAY, ${station}, ${callsign} declaring emergency. ${nature}. Requesting immediate priority.' },
                inf_pan: { label: 'PAN-PAN', text: 'PAN-PAN PAN-PAN PAN-PAN, ${station}, ${callsign}. ${nature}.' },
                req_priority_landing: { label: 'Request Priority Landing', text: '${station}, ${callsign} requesting priority landing runway ${runway}.' },
                req_emergency_descent: { label: 'Emergency Descent', text: 'Emergency descent, ${callsign}. Descending to ${altitude}.' },
                report_failure: { label: 'Report Failure', text: '${station}, ${callsign} reporting ${failure} failure.' },
                inf_fuel_emergency: { label: 'Fuel Emergency', text: 'MAYDAY, ${callsign}, fuel emergency. Remaining endurance ${minutes} minutes.' },
                req_crash_crew: { label: 'Request Crash Crew', text: '${station}, ${callsign} requesting crash crews on standby.' }
            }
        },
        utils: {
            title: 'Predictions',
